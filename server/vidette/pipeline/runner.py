...
from vidette.core.config import CameraConfig, PowerProfile, VidetteConfig
...
_BACKOFF_MAX_S = 60.0
_REPEAT_BACKOFF_MAX_S = 300.0  # battery profile: stop hammering a sleeping camera awake
    ...
    async def _supervise(self) -> None:
        ...
        while not self._stopping.is_set():
            ...
            if await self._backoff_wait(backoff):
                break
            # Battery profile only: a long failure streak usually means the camera sleeps —
            # stretch the retry interval so we stop waking it (and stop leaking gateway
            # sessions). Mains cameras (default) always retry within _BACKOFF_MAX_S.
            battery = self.camera.power_profile is PowerProfile.battery
            cap = (
                _REPEAT_BACKOFF_MAX_S
                if battery and consecutive_failures >= 3
                else _BACKOFF_MAX_S
            )
